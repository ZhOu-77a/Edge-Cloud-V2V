    ...
    def prepare_latents(
        ...
        if return_video_latents or (latents is None and not is_strength_max):
            ...
            def print_stats(name, tensor):
                """打印 Tensor 统计信息"""
                print(f"   🔍 [{name}] Shape: {tensor.shape} | Min: {tensor.min():.4f} | Max: {tensor.max():.4f} | Mean: {tensor.mean():.4f} | Std: {tensor.std():.4f}")
            print_stats("Encoded Latents (To Send)", video)
            # 检查一下 shift_factor 是否存在
            print(f"🕵️ [Pipeline Truth] Shift Factor in Config: {getattr(self.vae.config, 'shift_factor', 'Not Found')}")
        ...
        def print_stats(name, tensor):
            """打印 Tensor 统计信息"""
            print(f"   🔍 [{name}] Shape: {tensor.shape} | Min: {tensor.min():.4f} | Max: {tensor.max():.4f} | Mean: {tensor.mean():.4f} | Std: {tensor.std():.4f}")
        print_stats("Encoded Latents (To Send)", latents)
