...
logger.info("🚀 Client Experiment Script initialized on GPU 4.")
...
# 【修改 3】更改端口，避免与 GPU 0/1 的实验冲突 (原为 12345)
SERVER_PORT = 12346 
# 【修改 4】更改 Server 日志名
...
DEBUG_INPUT_DIR = "debug_inputs_check_gpu34"
...
def run_single_trial(video_path, video_name, fps, steps, cfg, seed, save_video=False):
    ...
    # 保存输入视频检查 (仅当需要保存视频且是第一轮时)
    if save_video:
        debug_save_path = os.path.join(DEBUG_INPUT_DIR, f"input_check_{video_name}_fps{fps}.mp4")
        if not os.path.exists(debug_save_path):
            try:
                ...
                iio.imwrite(debug_save_path, debug_vid, fps=fps)
            except Exception as e:
                logger.warning(f"   ⚠️ Failed to save debug input: {e}")
...
def plot_all_results(df_raw):
    try:
        ...
        plt.tight_layout()
        # 【修改 6】更改图片保存名
        plt.savefig("qoe_main_3x3_averaged_gpu34.png")
        ...
        plt.tight_layout()
        # 【修改 7】更改图片保存名
        plt.savefig("qoe_quality_breakdown_averaged_gpu34.png")
