...
for N in N_LIST:
    ...
    for ratio in RATIO_LIST:
        ...
        for m in M_LIST:
            ...
            print(f"\n🚀 Running Exp: {exp_id} (Actual steps: {ACTUAL_STEPS}, Interrupt at: {n_interrupt})")
            ...
            with torch.no_grad():
                ...
                while curr_idx < len(timesteps):
                    t_val = timesteps[curr_idx]
                    
                    # 动态中断逻辑
                    if not is_fast_mode and curr_idx == n_interrupt:
                        if len(timesteps) - curr_idx >= m:
                            print(f"🚨 [INTERRUPT] Re-scheduling {len(timesteps)-curr_idx} steps into {m} steps.")
                            # 更新后的重采样逻辑：包含上一步 index 以便更均匀
                            indices = np.linspace(curr_idx-1, len(timesteps) - 1, m+1, dtype=int)[1:]
                    ...
                    latent_model_input = scheduler.scale_model_input(latent_model_input, t_val)
                    ...
                    noise_pred = transformer(
                        ...
                        timestep=t_val.expand(latent_model_input.shape[0]),
                    ...
                    latents_curr = scheduler.step(noise_pred, t_val, latents_curr).prev_sample
                    curr_idx += 1
