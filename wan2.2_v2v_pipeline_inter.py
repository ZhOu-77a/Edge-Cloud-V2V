# wan2.2_v2v_task, execute cfg depend on the cfg_ratio. 
...
CFG_STOP_INDEX      = int(ACTUAL_TOTAL_STEPS * CFG_RATIO)
...
def execute_steps(model, steps_list, current_latents):
    """纯推理函数，执行传入的 steps_list, 包含cfg_ratio逻辑。"""
    if len(steps_list) == 0:
    ...
    while idx < len(steps_list):
        ...
        if check_interrupt():
            ...
            # 转换格式为 [(0, t), (0, t)...] 供主循环使用,我们只关心t
            new_future_steps = [(0, nt) for nt in new_steps_tensor]
            
            interrupted = True
            global_state["is_interrupted"] = True
            print("   -> 📉 Switching to Non-CFG mode (Guidance Scale = 1.0) for remaining steps.")
            # 即使中断，当前步 t 仍需跑完

        # --- 2.动态决定 CFG  ---
        # 条件 1: 基础 Scale > 1.0
        # 条件 2: 没有被中断 (中断后必须 No-CFG)
        # 条件 3: 当前步数 < CFG_STOP_INDEX (CFG Ratio 限制)
        
        current_step = global_state["current_step_count"]
        is_in_ratio_range = current_step < CFG_STOP_INDEX
        
        do_cfg = (GUIDANCE_SCALE > 1.0) and (not global_state["is_interrupted"]) and is_in_ratio_range

        # --- 3. 准备输入 ---
        if do_cfg:
            latent_model_input = torch.cat([current_latents] * 2)
            step_context = context_input
            step_y = y_model_input
        else:
            latent_model_input = current_latents
            step_context = [context_input[-1]] 
            step_y = y_model_input[:1]      
        
        timestep = t.expand(latent_model_input.shape[0])
        
        # --- 4. 模型推理 ---
        with torch.no_grad():
            noise_pred = model(x=latent_model_input, context=step_context, t=timestep, seq_len=seq_len, y=step_y)
        ...
        # 打印日志
        status_tag = f"(CFG: ON)" if do_cfg else f"(CFG: OFF)"
        print(f"      Step {global_state['current_step_count']} done {status_tag} (t={t.item():.4f}).")
...
save_path = "samples/inter_cfg/2.mp4"
